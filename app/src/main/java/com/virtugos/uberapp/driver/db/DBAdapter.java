package com.virtugos.uberapp.driver.db;
...
public class DBAdapter {
	...
	private static final String TABLE_CREATE_USER = "create table "
			...
			+ " text," + KEY_CAR_MODEL + " text," + KEY_CAR_NUMBER + " text);";
	...
	public long createUser(User user) {
		...
		values.put(KEY_CAR_MODEL, user.getCarModel());
	...
	public User getUser() {
		...
		if (cursor != null && cursor.getCount() > 0) {
			...
			user.setCarModel(cursor.getString(10));
			user.setCarNumber(cursor.getString(11));
			cursor.close();
		}
